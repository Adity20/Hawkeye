...
# Camera Object
cap = cv2.VideoCapture('video1.mp4')  # Number According to Camera
...
# Function to speak out the object name and distance
def speak_object_and_distance(object_name, distance):
    engine.say(f"The object is {object_name} and its distance is {distance} inches.")
    engine.runAndWait()
...
while True:
    ...
    if len(classIds) != 0:
        for i in indices:
            ...
            speak_object_and_distance(object_name, distance)  # Speak out the object name and distance
            cv2.rectangle(frame, (x, y), (x + w, y + h), color, thickness=2)
            cv2.putText(frame, object_name + " " + confidence, (x + 10, y + 20),
                        font, 1, color, 2)
